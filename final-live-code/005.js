...
function getGuildMemberInfo(members) {
    ...
    var obj = { totalMember: members.length }
    var jumlah = 0
    for (var i = 0; i < members.length; i++) {
        jumlah += members[i].level
    }
    var avg = Math.floor(jumlah / obj.totalMember)
    obj['averageLevel'] = avg
    for (var i = 0; i < members.length; i++) {
        var objTmp = {}
        if (obj[members[i].class] === undefined) {
            if (members[i].level >= obj.averageLevel) {
                obj[members[i].class] = []
                tmp = {
                    name: members[i].name,
                    level: members[i].level
                }
                obj[members[i].class].push(tmp)
            }
            // return obj
        } else {
            if (members[i].level >= obj.averageLevel) {
