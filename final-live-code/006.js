...
function oddPairFirstLast(num) {
    ...
    if (num === 0) {
        return 0
    } else if (String(num).length === 1 && num !== 0) {
        var baru = String(num) + '1'
        return Number(baru)
    } else {
        num = String(num)
        var tmp = Number(num[0] + num[num.length - 1]) //25
        var strBaru = Number(num.substr(1, num.length - 2)) //34
        if (tmp % 2 === 1) {
            return tmp + oddPairFirstLast(strBaru)
        } else {
            return oddPairFirstLast(strBaru)
        }
    }
